...
rdata<- subset(rdata, rdata$woba_denom==1)
...
heartzone<-geom_rect(data=NULL, mapping=aes(xmin=heartin, xmax=heartout, ymin=heartbot, ymax=hearttop),
                     size=1.5, alpha=0, color="green")

plotPitchesB = ggplot(rdata, aes(rdata$plate_x, rdata$plate_z, color=rdata$woba_value), size=.1)+
  scale_color_gradient2(low="blue", high="red", space="Lab", mid="white", midpoint=1, guide="colourbar", name="wOBA")+geom_point(size=1)+
  xlim(c(-4, 4))+ylim(c(-2, 6))+theme_bw()+
  shadowzone+strikezone+heartzone+chasezone

plotPitchesB

plotPitchesA = ggplot(rdata, aes(rdata$plate_x, rdata$plate_z, color=rdata$estimated_woba_using_speedangle), size=.1)+
  scale_color_gradient2(low="blue", high="red", space="Lab", mid="white", midpoint=1, guide="colourbar", name="xwOBA")+geom_point(size=1)+
  xlim(c(-4, 4))+ylim(c(-2, 6))+theme_bw()+
  shadowzone+strikezone+heartzone+chasezone
